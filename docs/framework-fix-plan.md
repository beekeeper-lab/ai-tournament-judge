...
Landed: D7 and D10. Added to the plan without landing: D12, D13.

D12 and D13 are not in the defect table above on purpose. That table is edited
by other branches; new rows are recorded here instead so the merge stays a
concatenation.
...
| D13 | Two more templates invite a `persona` value the registry can never resolve, and neither is caught as a placeholder | 1 | found, not landed |
...
## D13 — two more templates invite an unresolvable `persona` (found, not landed)
