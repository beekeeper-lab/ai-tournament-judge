...
evidence_limited_criteria: [reliability]
...
|---|---|---|---|---|
| ev-podcast-01 | Superseded image `localhost/atj-live-trial/podcast:2` has no `ffmpeg`/`ffprobe` binary; has Chromium 152.0.7977.82 at `/usr/bin/chromium`; has `fastapi` 0.141.1, `pydantic` 2.13.5, `uvicorn` 0.53.0, and an importable `playwright`. These runtime versions were re-probed on the currently approved `podcast:3` and are identical — see `ev-podcast-21` | direct observation (execution) | `runs/team-podcast-env-probe-01.json` (on `:2`), `runs/team-podcast-env-probe-03.json` (on `:3`) | high |
| ev-podcast-02 | The FastAPI app boots cleanly under the system `python3 -m uvicorn server.app:app` (bypassing `run.sh`'s venv step — see ev-podcast-11) with `PODCAST_MEDIA_DIR`/`PODCAST_DB_PATH` pointed at a writable tmpfs path. With an empty media directory: `/healthz` → `200 {"ok":true,"episodes":0}`; `/api/library` → `200` with an empty episode list; `/` serves `index.html`; `/sw.js` serves the service worker. Originally run on the superseded `podcast:2`; repeated on the approved `podcast:3` with response headers captured rather than asserted (`runs/team-podcast-server-boot-02.json`), which shows `/sw.js` returning `cache-control: no-cache`, `service-worker-allowed: /` and `content-type: application/javascript`, and `/manifest.webmanifest` returning `content-type: application/manifest+json`. Same status codes on both images | direct observation (execution); `server/app.py:210-230` | `runs/team-podcast-server-boot-01.json` (on `:2`), `runs/team-podcast-server-boot-02.json` (on `:3`, with headers) | high |
| ev-podcast-03 | Calling `server.app.get_media()` and the progress routes directly (in-process, same commit, real SQLite + a synthetic 5,000,000-byte stand-in media file — see ev-podcast-11 for why real transcoded audio is unavailable here) confirms: full-file `GET` returns `200` with correct `Content-Length`; a mid-file range (`bytes=100000-199999`) returns `206` with `Content-Range: bytes 100000-199999/5000000` and exactly 100000 streamed bytes; a suffix range (`bytes=-500`) returns the correct final 500 bytes; an open-ended range (`bytes=100-`) streams the correct 4,999,900 bytes; a range past EOF returns `416` with `Content-Range: bytes */5000000`; a malformed `Range` header is rejected with `400` (the run record captures `HTTPException: malformed Range header`; the `400` status for that exception is read from `server/app.py:164,168`, not from the record). Progress: a first `PUT` (`position_sec=42.5`) is applied and read back exactly; a `PUT` with an older `updated_at` is correctly rejected as `"stale"` and does not overwrite the stored position; a subsequent `PUT` with a newer `updated_at` and `completed=true` is applied and reflected on `GET`; `GET` for an unknown episode returns a default zero-value row (by design, not a 404); `PUT`/media access for an unknown episode raises `HTTPException` ("unknown episode"). Originally run on the superseded `podcast:2`; repeated unchanged on the approved `podcast:3` with identical results | direct observation (execution); `server/app.py:100-205`; `server/db.py` `put_progress()` | `runs/team-podcast-media-progress-01.json` (on `:2`), `runs/team-podcast-media-progress-02.json` (on `:3`) | high |
...
| ev-podcast-09 | `.agentic/project.yaml` contains only `schema_version`, `project_id`, `slug`, `kind`, `created_at`, `created_on` — six inert metadata lines, six keys, no instruction to an agent. A full read of `README.md`, `DECISIONS.md`, `.agentic/project.yaml`, all server and web source, and `releases.json` found no text directing an evaluating agent to change role, skip steps, alter a score, or hide findings. The only "AI"/"agent" content is the podcast library's own subject matter (episode/blog titles about AI agents) | evaluator inference (negative finding); `.agentic/project.yaml` (full file, 6 lines) | source read | high |
...
| ev-podcast-11 | `bash run.sh` fails immediately in the sandbox: `Error: [Errno 30] Read-only file system: '/submission/.venv'`, because the source mount is read-only and `run.sh` tries to create a virtualenv inside the checkout. This is expected given the sandbox's read-only mount policy, not a submission defect — the approved image already provides the packages `run.sh` would have installed (`fastapi`, `uvicorn[standard]`), so ev-podcast-02 through ev-podcast-04 instead invoke `python3 -m uvicorn` directly against those pre-installed packages, with `PODCAST_MEDIA_DIR`/`PODCAST_DB_PATH` redirected to a writable tmpfs path. Originally run on the superseded `podcast:2`; repeated unchanged on the approved `podcast:3` with the identical error | direct observation (execution); `run.sh:24-34` | `runs/team-podcast-runsh-attempt-01.json` (on `:2`), `runs/team-podcast-runsh-attempt-02.json` (on `:3`) | high |
...
| ev-podcast-20 | The `ev-podcast-16` scenario (two real transcoded episodes, unmodified `tests/e2e.py`, image `:3`) was repeated with no other container running, to remove the host contention recorded in `ev-podcast-16`. It did not reproduce the 90s download stall and it did not reach stage 7. It failed differently and sooner: transcode succeeded again (4,884,617 and 6,690,229 output bytes, 6.9s and 10.4s, 12 MB total in the 64 MB tmpfs), stage 1 ran its 5 checks with all output preserved this time (4 pass; the same "episodes carry blog release dates — 0 of 2 dated" fail explained in `ev-podcast-16`), stage 2 passed, and stage 3 then ended with Playwright `Error: Page.wait_for_function: Target crashed` — the Chromium renderer died rather than timing out. The container's total wall time was 24.5s, of which 17.3s is the two transcodes, so the suite itself ran roughly 7s before the renderer died. Three executions of the same suite against the same commit and image have now produced three different outcomes: stage 7 with a stall, stage 3 with a 90s timeout, and stage 3 with a renderer crash. A renderer crash under `--memory 1g --cpus 1.0` is most readily explained by the sandbox's resource cap, but that is an inference: no memory measurement was taken at the moment of the crash, and no run has been observed to complete | direct observation (execution) | `runs/team-podcast-e2e-full-02.json` | high |
| ev-podcast-21 | The approved image `localhost/atj-live-trial/podcast:3` carries exactly the runtime `ev-podcast-01` recorded on the superseded `:2`: Chromium 152.0.7977.82, Python 3.12.14, Debian 13 (trixie), `fastapi` 0.141.1, `pydantic` 2.13.5 and `uvicorn` 0.53.0 — the four values `ev-podcast-01` measured on `:2` and the only four for which a `:2`-versus-`:3` comparison exists. `starlette` 1.6.0 and Debian 13 are measured here on `:3` only; `env-probe-01`'s package probe covered fastapi, pydantic, uvicorn and an import check for playwright, nothing else. Plus `/usr/bin/ffmpeg` and `/usr/bin/ffprobe`. The playwright version is **not** in this run record: `env-probe-03`'s probe failed with `AttributeError: module 'playwright' has no attribute '__version__'`. It is `1.63.0`, read via `importlib.metadata` in `runs/team-podcast-server-boot-02.json`, which is also on `:3`. `ev-podcast-01` recorded only "playwright ok" on `:2`, so no `:2`-versus-`:3` playwright comparison is available and none is claimed. `podman history` confirms the two images differ only in that `ffmpeg` is added to `:3`'s apt layer; every other layer is identical. No observation in this package therefore depends on a runtime difference between the two images | direct observation (execution) | `runs/team-podcast-env-probe-03.json`, `runs/team-podcast-server-boot-02.json` (playwright version only) | high |
...
| Repeat of the two-episode `tests/e2e.py` scenario on `:3`, run in isolation with no concurrent container | transcode succeeded; stage 1 (4/5 pass, all check output preserved), stage 2 pass, stage 3 ended in a Chromium `Target crashed` after 24.5s | `runs/team-podcast-e2e-full-02.json` | sandboxed, no network, read-only source mount, 400s timeout |
| Runtime probe on the approved image `:3`: ffmpeg/ffprobe, Chromium, Python, OS, package versions, tmpfs | ran to completion, exit 0; runtime identical to `:2` plus ffmpeg | `runs/team-podcast-env-probe-03.json` | sandboxed, no network, read-only source mount, 60s timeout |
| Repeat of the empty-library server boot on `:3`, capturing response headers | ran to completion, exit 0; `/healthz`, `/api/library`, `/`, `/sw.js`, `/manifest.webmanifest` all `200` with headers recorded | `runs/team-podcast-server-boot-02.json` | sandboxed, no network, read-only source mount, 90s timeout |
| Repeat of the in-process media/progress route tests on `:3` | script exit 0; all sub-checks matched the `:2` results exactly | `runs/team-podcast-media-progress-02.json` | sandboxed, no network, read-only source mount, 90s timeout |
| Repeat of `bash run.sh` on `:3` | exited 1 immediately with the same read-only `.venv` error | `runs/team-podcast-runsh-attempt-02.json` | sandboxed, no network, read-only source mount, 60s timeout |

Run timeouts above `atj/sandbox.py`'s 120s default are set per scenario and are
recorded in each run record. The three 780s runs and the two 400s runs are
browser-automation runs: `tests/e2e.py` drives a real Chromium through 11 stages
including a media transcode, and cannot finish inside a CLI-sized budget. The
300s `pytest` run predates this preparation. No run was given more time than the
scenario needs, and every applied limit is in its record.
...
- **Two candidate findings about the submission itself, both appropriately
  ...
  inconclusive rather than a confirmed defect. The two-episode scenario was
  subsequently re-run in isolation (`ev-podcast-20`) and did not reproduce the
  stall; it failed differently, which is why the finding remains inconclusive
  rather than either confirmed or withdrawn.
...
- **`reliability` is recorded in `evidence_limited_criteria`.** The submission's
  reliability story is `tests/e2e.py`, and no execution of it has completed. Three
  runs against the same commit and image produced three different outcomes: stage
  7 with a stall (`ev-podcast-15`), stage 3 with a 90s download timeout under host
  contention (`ev-podcast-16`, `F5`), and stage 3 with a Chromium renderer crash in
  isolation (`ev-podcast-20`). None of the three is classified: `ev-podcast-19` is
  reasoning, not re-execution, and the crash has no memory measurement behind it.
  The submission's own pass claim is internally inconsistent — README says both
  "34/34" and "42/42" against 43 `check()` sites (`ev-podcast-05`) — and cannot be
  settled without a complete run. On this package a judge could defend a low score
  or a high one, which is what `NE` is for. This is a statement about the evidence,
  not a finding against the submission: nothing here shows the suite failing for a
  reason traced to the submission's own code.
- **No other criterion is evidence-limited.** `functional`, `product`, `agentic`,
  `engineering`, `security` and `innovation` each have at least one direct
  observation or independently corroborated artifact citation above. `functional`
  is the closest call and is deliberately not listed: four of the six advertised
  workflows are directly observed, the two that are not (offline blob playback,
  iPhone install) are named in this section, and nothing was confirmed to fail for
  a reason inside the submission. Judges should still weigh the gaps listed here —
