...
| 2026-09-17T22:50:00Z | Bracket built by `atj bracket build`, seed `live-trial-2026`, roster version 1, input digest `5bd7a2f7e711e721`. Single-elimination, 2 entrants, 1 final, 0 byes. Of the constraints evaluated, four are `satisfied` and one (previous champion and runner-up in opposite halves) is `not-applicable` with no previous finalists; affiliation separation is a soft constraint and also not-applicable with one team per group (audit B4). Rebuilt from the same seed and byte-identical, and `atj bracket verify` re-derived the constraints from the roster: PASS. The `performance-qualified` bye policy never reads a total here because two teams is an exact power of two and produces no byes — which is why team-podcast's missing total does not block the bracket, as the adjudication predicted | teams.md (frozen v1), event.md | bracket.json | not-audited |
| 2026-09-17T23:05:00Z | Bracket stage audited by `judging-auditor@1.0.0`, FIRST pass, scope rule applied: PASS WITH ADVISORIES, no blocking finding. The auditor proved the no-score claim two independent ways — by code path, and by rebuilding the draw under five score permutations including team-podcast's unofficial 58.25, all of which left `rounds` and `constraint_audit` bit-identical | bracket.json, teams.md v1, event.md, bracket-assignment.md, both summaries | audits/bracket.md | PASS WITH ADVISORIES |
| 2026-09-17T23:10:00Z | B2-B6 repaired: `bracket.json` rebuilt so `framework_commit` is a real commit rather than `uncommitted`; `bracket.md` regenerated from `bracket.json` by script rather than transcribed, closing the B3 stub that still read `status: not-built`; the constraint count corrected; two stage-gate checkboxes ticked to match their own front matter. D20 and D21 logged | audits/bracket.md | bracket.json, bracket.md, status.md, docs/framework-fix-plan.md | operator-verified |
| 2026-09-17T23:10:00Z | FRAMEWORK DEFECT D20, Tier 1, found by the bracket audit: `atj event validate` returns FAIL for this event because `atj/event.py:356-364` demands a consolidated score for every eligible team whenever `bye_policy` is `performance-qualified` and the stage is at or past `bracket` — without asking whether any bye exists. Here there are none. The two repairs available inside the event are fabricating a total for team-podcast, which is the offence the audit exists to prevent, or changing `event.md`'s bye policy mid-event, which CLAUDE.md forbids. A rule whose only in-scope repairs are both blocking offences is a defect in the framework, not in the event | `python3 -m atj event validate events/live-trial-2026` | docs/framework-fix-plan.md | not-audited |
| 2026-09-17T23:35:00Z | Head-to-head judged in BOTH presentation orders by `matchup-judge@1.0.0`, the two passes run concurrently and blind to each other, each staged under `workspaces/live-trial-2026/staging/matchup/` before promotion. Resolved by `atj matchup`: **team-ledger advances**, combined margin **+35.00** on -100..+100, outcome `confirmed`, no order disagreement on any of the seven criteria, no adjudication required. A-first margin +32.50, B-first normalized +37.50; both passes picked team-ledger independently. Neither judge computed a margin and both predicted qualitatively that the result would land near the close-call band of 5 — it landed at seven times the band, which is the case for keeping arithmetic in the tool | ev packages for both teams, both consolidated reports, the eight judgments, both checkouts | matchups/mu-final-01.json, matchups/mu-final-01.md, matchup-passes/*.md | not-audited |
| 2026-09-17T23:35:00Z | Comparison values verified against each judge's own returned output before resolution: both matched exactly. `atj/matchup.py:42-70` performed the B-first negation from `presented_first`; no judge and no operator normalized by hand | matchup-passes/*.md | matchups/mu-final-01.json | not-audited |
| 2026-09-17T23:40:00Z | `atj bracket advance` recorded team-ledger into `bracket.json` from the private matchup report; `ready to judge next: none`. The tournament has no further matches | matchups/mu-final-01.md | bracket.json, bracket.md | not-audited |
| 2026-09-17T23:40:00Z | FRAMEWORK DEFECT D22: `schemas/matchup.schema.json` requires both passes' `comparisons` to be non-empty, so an order-balanced pass report — whose empty second block IS the evidence of independence — can never validate, and the validator recurses so a subdirectory of `matchups/` does not help. Both judges' reports were moved to `events/live-trial-2026/matchup-passes/`, outside any declared `EVENT_SUBDIRS`, purely so the event would validate | `atj validate reports` | docs/framework-fix-plan.md | not-audited |
| 2026-09-18T00:20:00Z | Tournament stage audited by `judging-auditor@1.0.0`, FIRST pass with the scope rule: PASS WITH ADVISORIES, no blocking finding. The resolution was reproduced independently — each judge's raw values transcribed from that judge's own report, re-run through `atj matchup`, and full-key-diffed against the committed JSON with no differing, missing or extra key. Independence re-tested by subtracting every upstream document's n-grams from the shared n-grams of the two passes; everything surviving resolved to a source. The truncated first extraction of the a-first report was confirmed a pure append: `diff` reports `118a119,433` and nothing else | matchups/*, matchup-passes/*, bracket.json, bracket.md, both summaries | audits/tournament.md | PASS WITH ADVISORIES |
