...
evidence_package_id: ev:trial-2-2026:team-demos:dc35f6962130:1c0b2b5e
...
- **The event's decision for the dry run**, owed by the intake record and taken
  ...
  dry-run early return (`01-resume-that-talked-back/demo/rank_resumes.py:93-110`),
...
| ev-demos-07 | The egress guards were exercised directly against hostile URLs. `fetch_beacons.is_localhost` admits `127.0.0.1`, `localhost` and `[::1]` and refuses `evil.example`, `localhost@evil.example` (userinfo confusion), `127.0.0.1.evil.example` (suffix confusion), `0.0.0.0`, the decimal form `2130706433` and `file:///etc/passwd`; its `ALLOWLIST` is empty, so with `--enforce-allowlist` even a localhost beacon is refused. `resume_parser/v1.1`'s `_post_to_localhost` raises `RuntimeError: blocked: resume-parser refuses to POST to non-localhost host` for `evil.example` and for the userinfo form. The guards are real controls, not comments | R5 | `07-what-the-output-smuggles-out/demo/fetch_beacons.py:42-67,83-90`; `09-toolbox-you-didnt-audit/demo/tools/resume_parser/v1.1/parser.py:90-107` | `runs/team-demos-egress-guards-01.json` | High |
...
| ev-demos-13 | Provenance correction, recorded because a later artifact would otherwise inherit it: commit `3f484d5`'s message and `Containerfile.demos`'s header both say "all 19 demo scripts". The tree holds 53 `.py` files — 17 demo-root entry points, 17 under `scripts/`, 9 under `hardened/`, 8 under `tools/`, and 2 parser versions. Nineteen is the number of files that import `anthropic`, not the number of scripts. The import conclusion those artifacts drew is correct; the count attached to it is not | - | `Containerfile.demos`; commit `3f484d5` | AST scan against the pinned checkout | High |
...
| `python3 -c '<egress guard probe>'` — `is_localhost` and `_post_to_localhost` against nine hostile URLs | exit 0; every non-local form refused | `runs/team-demos-egress-guards-01.json` | as above |
...
|---|---|---|
| ev-demos-04 | `01-resume-that-talked-back/demo/rank_resumes.py:37-47` | The vulnerable system prompt: the taught defect, in text |
| ev-demos-05 | `01-resume-that-talked-back/demo/hardened/rank_resumes_hardened.py` | The hardened system prompt: the taught fix, in text |
...
| ev-demos-02 | `01-resume-that-talked-back/demo/rank_resumes.py:93-110` | `import anthropic` below the dry-run return — why a dry run needs no SDK |
