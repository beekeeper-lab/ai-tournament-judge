...
1. The primary documented run path deliberately gets an agent hijacked on the presenter's own machine while `Write` and `Bash(python3:*)` are pre-approved in that session (`01-…/.claude/commands/rank-resumes.md:4`, and the same pattern in 02, 03, 06, 07, 09, 10). No README in the tree tells the presenter to run that session in a container, a throwaway clone, or anything else — a grep for isolation language across all READMEs returns only demo 09's in-process tool sandbox. The payloads are stated to be benign props, and I saw nothing contradicting that; the gap is that the corpus teaches blast-radius discipline everywhere except in its own run instructions.
