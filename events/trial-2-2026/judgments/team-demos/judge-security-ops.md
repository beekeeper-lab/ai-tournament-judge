...
**Deficiencies.** None I can confirm against the pinned tree. I had recorded the documented setup step `cp .env.example .env` as broken in every demo; a stage audit challenged that and the challenge is correct. The checkout's git index tracks ten `.env.example` files at the pin, `01-resume-that-talked-back/demo/.env.example` through `10-show-your-work/demo/.env.example`, one inside each demo folder — which is the folder every README runs the copy from (`01-resume-that-talked-back/demo/README.md:15`, "Everything runs from **this `demo/` folder**"; root `README.md:16,23`, which sends the reader into `NN-slug/demo/` before the copy at `:19`). The documented setup path resolves in all ten demos. What is left here is an evidence limit rather than a defect: nine of the ten demos have no proof path that runs without a model, which is what the improvement below addresses.
