...
**D1 is withdrawn**, on a stage audit finding against this file. It claimed no `.env.example` exists anywhere at the pin. Ten do: the checkout's git index tracks `01-resume-that-talked-back/demo/.env.example` through `10-show-your-work/demo/.env.example`, one in each demo folder, which is the folder every README runs `cp .env.example .env` from. My original check was a filesystem glob for `**/.env*`, and it returned nothing because this environment excludes `.env*` paths from search results and refuses to read them — a secrets guard, which I read as a fact about the submission. The demo `.gitignore` files were not the cause; they list `.env`, which does not match `.env.example`. The numbering below is left as D2 through D6 so the cross-references in K1 still resolve, and nothing in D2 through D6 or K1 through K4 depended on D1.
