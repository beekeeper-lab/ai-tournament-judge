...
- [x] Passages reissued after the consolidation stage audit, 2026-09-22: a third uncorrected "seven" survived the second reissue, in the executive assessment, and is now seventeen, consistent with the enumeration the audit re-derived file by file (consolidation audit F15). In the same paragraph, the claim that the v1.1 `_post_to_localhost` guard raised for every non-local host is narrowed to what `runs/team-demos-egress-guards-01.json` records: it raises for `evil.example` and the userinfo form, and the schemeless form is rejected by `urllib` instead of by the guard, which is this judgment's own D-series point about the empty-hostname hole. Corrected by the event director with no judge re-run; no score, confidence or anchor moved.
