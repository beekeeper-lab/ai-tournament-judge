...
**Deficiencies.** None I can confirm against the pinned tree. I had recorded the documented setup step `cp .env.example .env` as broken in every demo; a stage audit challenged that and the challenge is correct. The checkout's git index tracks ten `.env.example` files at the pin, `01-resume-that-talked-back/demo/.env.example` through `10-show-your-work/demo/.env.example`, one inside each demo folder — which is the folder each demo README runs the copy from (`01-resume-that-talked-back/demo/README.md:15`, "Everything runs from **this `demo/` folder**"), and the folder the root README sends the reader to for the presenter script (`README.md:23`, "Each talk lives in `NN-slug/demo/` and runs standalone"). The documented setup path resolves in all ten demos. One ordering nit, corrected on a second audit against an earlier version of this paragraph: the root README gives the copy inside its Python bullet at `README.md:19` without naming a folder, the `cd` at `:16` belongs to the Claude Code bullet above it, and the scoping sentence at `:23` comes after. That is a sequencing wrinkle in one file rather than a defect, since no script for run path B exists at the repository root — all 53 `.py` files sit under `NN-*/demo/` — so the root is never a working directory for that path. What is left here is an evidence limit rather than a defect: nine of the ten demos have no proof path that runs without a model, which is what the improvement below addresses.
...
**Deficiencies.** None confirmed. The one recorded here — a missing `.env.example` breaking the first line of the documented API path — is withdrawn on audit; the correction is set out under `functional` above and as D1 below. The tree ships a `.env.example` inside each of the ten `demo/` folders, beside a `.gitignore` whose first line is `.env` (`01-resume-that-talked-back/demo/.gitignore:1`, `09-toolbox-you-didnt-audit/demo/.gitignore:1`). A template next to the ignore rule, per demo, is the arrangement that keeps a key off the stage and out of the repository, and this submission has it in all ten places.

**Score rationale.** Strong and clearly above normal for a prototype event: the artifact is designed around its actual user and shows it in details a first draft does not have. The score stays at 4 after the correction, and it now rests on one reason instead of two. The reason that survives is coverage of my own reading: I read five of the ten presenter scripts end to end (01, 05, 06, 07, 09) and know the other five only through ev-demos-02's structural count of ten directories and ten READMEs. The top anchor asks for that quality to be evident across the artifact, and half of this artifact I have counted rather than read. That is a limit on my evidence, not a fault found in the submission, and it is why confidence here stays medium rather than rising with the corrected fact. Had the setup step been the only thing holding the score down, the correction would have moved it.

**Uncertainty.** Medium: half the presenter scripts were confirmed to exist but not read by me.

**Highest-value improvement.** Document the no-network invocation beside the `uv` one. Every README gives `uv run --with anthropic python <script> --dry-run`, which resolves a package from PyPI before it prints anything; `import anthropic` sits below the dry-run early return, so plain `python3 <script> --dry-run` produces the same output with no install and no network (`01-resume-that-talked-back/demo/rank_resumes.py:103-109`; manifest, "The event's decision for the dry run"). One extra line per README is the difference between a rehearsal that survives a conference network and one that does not.
...
**Strengths.** The guards are controls, not comments, and they hold against input shapes most hand-rolled localhost checks fail on. Attacker-controlled strings that reach the filesystem are slugified to `[a-z0-9-]` before being joined to a fixed directory (`05-reading-is-safe-calling-is-not/demo/tools/_state.py:51-56` used by `tools/send_email.py:22-24`; `tools/resume_parser/v1.1/parser.py:113`), so a payload cannot traverse out of `outbox/` or `exfil-log/`. Every path constant is anchored to the demo directory via `Path(__file__).resolve()`. Key handling is clean. Each demo folder ships a `.env.example` template next to a `.gitignore` whose first line is `.env` (`01-resume-that-talked-back/demo/.gitignore:1`; ten such templates tracked at the pin), the root `.gitignore` covers `.env` and `**/.env` (`.gitignore:1-3`), and the generated artifacts that would hold synthetic candidate data are ignored. The key is read from the environment and never printed into a report (`01-resume-that-talked-back/demo/rank_resumes.py:112-116`).
...
**Worse than expected.** That the corpus's own standard is not applied uniformly to its two run paths. Demo 06 spends a README telling the audience a prompt is not a ceiling, then in the path it calls the hero path implements the ceiling as an instruction in a prompt the demo has just told the model to let untrusted resumes steer. Related and unexpected: the breadth of `Bash(rm:*)`, carried by seventeen command files, with at least one in every one of the ten demos and three in demo 10 — including `screen-pile-audited` (`10-show-your-work/demo/.claude/commands/screen-pile-audited.md:4`), where the only rm the command needs is `rm -f reports/decisions/*.json` on line 30 — a demo about auditability granting an unaudited delete.
...
**D1 is withdrawn**, on a stage audit finding against this file. It claimed no `.env.example` exists anywhere at the pin. Ten do: the checkout's git index tracks `01-resume-that-talked-back/demo/.env.example` through `10-show-your-work/demo/.env.example`, one in each demo folder, which is the folder each demo README runs `cp .env.example .env` from; the root README's own copy line at `README.md:19` names no folder, which is the ordering nit recorded under `functional` and not a defect, since run path B has no script at the repository root. My original check was a filesystem glob for `**/.env*`, and it returned nothing because this environment excludes `.env*` paths from search results and refuses to read them — a secrets guard, which I read as a fact about the submission. The demo `.gitignore` files were not the cause; they list `.env`, which does not match `.env.example`. The numbering below is left as D2 through D6 so the cross-references in K1 still resolve, and nothing in D2 through D6 or K1 through K4 depended on D1.
- **D2. Tool pre-approvals exceed the actions they cover.** `Bash(rm:*)` is pre-approved in seventeen command files, corrected on audit from the eight this entry first listed, and every one of the ten demos carries at least one. Paths are relative to `NN-*/demo/.claude/commands/`: `01/reset-demo.md:3`, `01/unstage-goofy.md:3`, `02/reset-demo.md:3`, `02/unstage-hidden.md:3`, `03/reset-demo.md:3`, `04/reset-demo.md:3`, `05/unstage-action-resume.md:3`, `06/reset-demo.md:3`, `06/unstage-poison.md:3`, `07/reset-demo.md:3`, `07/unstage-beacon.md:3`, `08/reset-demo.md:3`, `08/unstage-injection.md:3`, `09/reset-demo.md:3`, `10/reset-demo.md:3`, `10/unstage-attacks.md:3` and `10/screen-pile-audited.md:4`. The rm each command actually runs is a fixed, narrow one (for example `rm -f resumes/goofy-goof.md reports/RANKING.md`, `01/reset-demo.md:7`). `Bash(python3:*)` is likewise pre-approved in the acting commands. The corrected count widens this entry rather than softening it: a broad delete grant is the corpus's default for any command that removes a file, not an oversight in a few. It also makes the fix cheaper to argue, because the tree already holds the counter-example — `05/reset-demo.md:3` grants `Bash(python3:*), Bash(ls:*)` and no rm, delegating the deletes to `python3 scripts/reset_demo.py` (`:7`).
...
- [x] Passages reissued after a stage audit, 2026-09-22: the audit challenged D1 and was right, because checking the checkout's git index rather than a filesystem glob shows ten tracked `.env.example` files, one in each `NN-*/demo/` folder where the READMEs run the copy, and my original glob returned nothing only because this environment hides and refuses `.env*` paths as a secrets guard, so a present tracked file read to me as an absent one; no other judge's work was seen in the correction, no front-matter score changed, and the `atj:scores` block was left untouched.
- [x] Passages reissued after a second stage audit, 2026-09-22: `Bash(rm:*)` is pre-approved in seventeen command files spread across all ten demos rather than the seven and eight this file gave in two places, which widens D2 rather than softening it, and the root README citation inside the D1 withdrawal was wrong on ordering — `README.md:19` gives the copy inside the Python bullet, the `cd` at `:16` scopes only the Claude Code bullet above it, and the folder statement at `:23` follows rather than precedes it — so that citation now rests on `README.md:23` and `01-resume-that-talked-back/demo/README.md:15`, with the D1 withdrawal itself unaffected and no raw score moved.
- [x] Passages reissued after the consolidation stage audit, 2026-09-22: a third uncorrected "seven" survived the second reissue, in the executive assessment, and is now seventeen, consistent with the enumeration the audit re-derived file by file (consolidation audit F15). In the same paragraph, the claim that the v1.1 `_post_to_localhost` guard raised for every non-local host is narrowed to what `runs/team-demos-egress-guards-01.json` records: it raises for three of five forms, and the bare `evil.example/collect` is rejected by `urllib` instead of by the guard, which is this judgment's own K4 about the empty-hostname hole. The consolidation audit's re-audit caught the first narrowing itself undercounting the raises at two; three is what the run record shows. Corrected by the event director with no judge re-run; no score, confidence or anchor moved.
