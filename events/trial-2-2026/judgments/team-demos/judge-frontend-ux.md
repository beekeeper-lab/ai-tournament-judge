...
model:
  ...
  completed_at: "2026-09-22T10:43:34Z"
  verified: true
  note: Model identity is harness-reported for this judge run, not a self-report by the model. No independent attestation was available inside the evidence package, and none is claimed.
...
Where I can see the surface directly, I found a cluster of small breaks on the operator's path and no large one. The CLI error and empty-state strings that a path-B user will hit name path-A slash commands they do not have, and the top-level README gives no troubleshooting, no minimum Python version and no keyless rehearsal route. A larger defect I reported in the first issue of this judgment — that `cp .env.example .env` names a file absent from the tree — was my own error and is withdrawn. The checkout's git index tracks `NN-*/demo/.env.example` for all ten demos, so the first command of documented run path B resolves. My original check used a filesystem glob, which does not list dotfiles, and my reader refuses `.env*` paths outright, so the file was invisible to both tools I used; the `.gitignore:2-3` reasoning was correct and irrelevant. I cannot read the file's contents under the same rule, so I make no claim about what it contains.
...
**Deficiencies.** Three, all on the operator's path. First, error and empty-state strings name only Claude Code commands even when the user is on the Python path: `render_ranking.py:240` says "run /rank-resumes first", `07/demo/scripts/render_assessment.py:169-170` says "run /assess-candidate first", and `10/demo/scripts/list_verdicts.py:20,40` names `/screen-pile` and `/explain` although the same README gives the path-B equivalent at `10/demo/README.md:40`. Second, the top-level README has no troubleshooting section, no stated minimum Python version, and no degraded-mode guidance (`README.md:12-24`), while the only documented keyless rehearsal path sits inside one demo's README (`10/demo/README.md:32-34`) where a presenter with a dead venue network will not find it; the same section's path-B line, "Copy `.env.example` to `.env`" (`README.md:19`), is the one of the eleven `.env.example` references whose own directory does not ship the file, since all ten tracked copies live in `NN-*/demo/`. A reader who follows `README.md:23` into a talk folder first is unaffected, and that is the documented way in. Third, the running-cast promise is looser than stated: "Marisol" appears by name in seven of ten demos (`ev-demos-12`), so the continuity the top README sells at `README.md:8-9` is partial.

**Score rationale.** The documentation and operator surface clearly exceeds what this event normally sees, and the claim is supported by direct observation across all ten demos rather than by a sample. That is the strong anchor, and it is where this lands. It does not reach the top anchor, and the reasons that hold it there do not include the withdrawn `.env.example` defect. The error strings actively misdirect users of the other path, sending a Python-path user who is already stuck to a slash command they do not have. The top-level README carries no troubleshooting, no minimum Python version and no degraded-mode guidance, so the presenter whose venue network is dead has nowhere to look. Above all, the product's real delivery surface is a live room watching an agent get hijacked, and none of it was observed in this event (`ev-demos-01`); the top anchor asks for unusually complete, effective and well-supported, and effectiveness on that surface has no evidence either way. The visual artifact was assessed from its generator only, not from rendered output, since `reports/` is gitignored and the tree is clean (`.gitignore:5-7`, `ev-demos-11`), so no contrast measurement or assistive-technology pass was possible. I have not credited or penalized rendered visual quality.
...
**Highest-value improvement.** Make every error string name both paths, for example "run `/rank-resumes` (Claude Code) or `python3 rank_resumes.py` (Python)". It is a three-file edit and it repairs recovery for exactly the user who is already stuck.
...
**Deficiencies.** Effectiveness is unobserved. No agent ran, so no control was seen to hold under attack (`ev-demos-01`, manifest "Missing or inaccessible evidence" items 1 and 4). Demo 06's approval queue and review flow were never exercised, and demo 10's audited screener has nothing to audit at the pin (`ev-demos-11`). Separately, the shipped agent configuration pre-approves tool use so the demo does not prompt mid-presentation, though not where I first said it does. The ranking commands pre-approve `Read, Glob, Write, Bash(python3:*)` and no `rm` (`01/demo/.claude/commands/rank-resumes.md:4`, `rank-resumes-hardened.md:4`). `Bash(rm:*)` is pre-approved in seventeen command files across all ten demos, and the READMEs say so plainly (`01/demo/.claude/commands/reset-demo.md:3`, `unstage-goofy.md:3`, `01/demo/README.md:94-95`, `06/demo/README.md:115-117`). Sixteen of those are staging or teardown commands whose bodies name specific files, which is defensible for a stage prop. The seventeenth is not teardown: `10/demo/.claude/commands/screen-pile-audited.md:4` is the capstone's audited screener, and it carries `Bash(rm:*)` alongside `Write`. That is the sharpest form of the point I was making — the corpus whose sixth talk teaches approval gates hands its most trust-centred command an unprompted delete. It is documented and defensible for a stage prop, and it is still the one place where the corpus's own lesson and its packaging point in different directions.
...
**Deficiencies.** Two guard paths were never exercised: `--enforce-allowlist`, which `ev-demos-07` notes would refuse even a localhost beacon, and the beacon server end to end (manifest "Tests and execution", final row). The v1.1 guard admits an empty hostname, which `urllib` then rejects, so it is narrower than the demo-07 guard without an observable consequence here (`ev-demos-08`). The shipped `.claude/` commands pre-approve `Bash(rm:*)` to avoid mid-demo prompts, in seventeen command files across all ten demos (`01/demo/.claude/commands/reset-demo.md:3`, `unstage-goofy.md:3`, documented at `01/demo/README.md:94-95`), one of which is not a teardown command but the capstone's audited screener (`10/demo/.claude/commands/screen-pile-audited.md:4`). The ranking commands themselves pre-approve only `Read, Glob, Write, Bash(python3:*)` (`rank-resumes.md:4`). A deliberate and documented trade that still narrows the human approval surface in a corpus that teaches approval gates.
...
**Worse than expected.** The cross-path error strings, in a corpus this careful about the operator everywhere else. A Python-path user who hits a failure is told to run a slash command that exists only in the other path (`render_ranking.py:240`, `render_assessment.py:169-170`, `list_verdicts.py:20,40`). Paired with the manifest's finding that the documented dry-run form resolves a PyPI package before printing anything (`ev-demos-01`, `ev-demos-02`), the run-path documentation is the weakest part of an otherwise exceptional documentation set. I originally recorded something worse here, that `cp .env.example .env` names a file absent from the tree. That was my tooling error, not a defect of the submission, and it is withdrawn: the git index tracks `.env.example` in all ten demo folders.

## Blocking and major issues

**Confirmed defects.**

1. **Withdrawn after a stage audit, and recorded here rather than deleted.** I reported that `.env.example` is referenced in all eleven READMEs and absent from the pinned tree, and that documented run path B therefore fails at its first step. It is not absent. The checkout's git index lists `NN-*/demo/.env.example` for all ten demos, so each demo README's `cp .env.example .env` resolves and that run path has no broken first step. My original check used a filesystem glob, which does not list dotfiles, and my reader refuses `.env*` paths outright; neither tool could see the file, and the `.gitignore:2-3` argument I built on top was sound reasoning about the wrong fact. I cannot read the file's contents under the same rule and claim nothing about them. What survives is minor and is scored under `product`: the top-level README's path-B line (`README.md:19`) is the one `.env.example` reference whose own directory ships no such file.
...
- [x] Passages reissued by this judge after a stage audit, 2026-09-22, re-checking both findings against the pinned checkout: `.env.example` is tracked in all ten demo folders and my confirmed defect was an artifact of tools that cannot see dotfiles, so it is withdrawn; the `rm` pre-approval is real but belongs to seventeen staging and teardown commands and to the capstone's audited screener, not to the ranking command I cited. No raw score changed, and no other judge's work was seen at any point. Recorded as a checkbox rather than as prose after this declaration, which is the W9 defect.
