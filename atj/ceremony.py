...
def _table(rows: list[str]) -> str:
    """Render a Markdown pipe table. Dropping them silently lost real content."""
    parsed = []
    for row in rows:
    ...
    lines = [f"<thead><tr>{head}</tr></thead>"]
    if body:
        cells = "".join(
            "<tr>" + "".join(f"<td>{_inline(cell)}</td>" for cell in row) + "</tr>"
            for row in body
        )
        lines.append(f"<tbody>{cells}</tbody>")
...
def bracket_view(event_dir: Path) -> dict[str, Any]:
    ...
    # A bracket file is an input like any other. Rendering it without validating
    # it first let a crafted field inject markup into the ceremony page.
    from . import schema

    problems = schema.validate("bracket", drawn)
    if problems:
        raise ValidationError(
            "refusing to render: bracket.json does not validate:\n  " + "\n  ".join(problems)
        )
    return {
...
def _results_block(sections: dict[str, str]) -> str:
    """The approved summary's own results table, if it published one."""
    results = sections.get("Bracket results", "").strip()
    if not results:
        return ""
    rendered = _blocks(results)
...
<p class="sub">{html.escape(str(bracket['team_count']))} teams,
{html.escape(str(bracket['bracket_size']))} slots,
{html.escape(str(bracket['bye_count']))} byes.
Current round: <strong>{html.escape(current_round)}</strong>.</p>
<div class="rounds">{"".join(round_blocks)}</div>

{_results_block(sections)}
...
def render_dossier(dossier_path: Path) -> str:
    ...
    findings = publication.check_team_facing(
        ...
        own_team=str(metadata.get("team_id") or dossier_path.stem),
    )
    blocking = [finding for finding in findings if finding.severity == "blocking"]
    if blocking:
        raise ValidationError(
