...
def consolidation_for(team: Team, *, resolved: bool, root: Path | None = None) -> dict[str, Any]:
    ...
    if resolved and team.id == "team-lumen":
        ...
        for judgment in judgments:
            ...
            if scores.get("security") == canon.NOT_ENOUGH_EVIDENCE:
                scores["security"] = LUMEN_ADJUDICATION["resolved_score"]
            adjusted.append(
                scoring.Judgment(
                    judge_id=judgment.judge_id, judge_run_id=judgment.judge_run_id,
                    team_id=judgment.team_id, commit=judgment.commit,
                    evidence_package_id=judgment.evidence_package_id, rubric=judgment.rubric,
                    persona=judgment.persona, scores=scores, confidence=judgment.confidence,
...
def expected_conditions(root: Path | None = None) -> dict[str, Any]:
    ...
    findings["twenty_team_fixture"] = {
        ...
        "reproducible": twenty_team_bracket(root)["rounds"] == fixture["rounds"],
    }
    return findings
